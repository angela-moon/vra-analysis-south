...
compile_tmp_files(tmp_files, "FL_dperc_party1.jld2", "FL_splits_party1.jld2", "FL_cuts_party1.jld2", "FL_bperc_party1.jld2")
exit()

#### PROCESSING DATA
using StatsPlots, Plots

cs_array = load("FL_splits_party1.jld2", "county_splits_arr")
percents_array = load("./FL_dperc_party1.jld2", "perc_arr")
cut_edges_array = load("FL_cuts_party1.jld2", "cut_edges_arr")
bpercs_array = load("FL_bperc_party1.jld2", "barr")

key_percs = hcat(percents_array[:,160_000:175_000,1],percents_array[:,320_000:end,1])

b = plot(title="% Black Box Plots FL; .45, .45, .45, .45 (less); .55 (greater)")
...
b = plot(title="% Dem Box Plots FL; .45, .45, .45, .45 (less); .55 (greater)")
...
plot(cut_edges_array, title="Cut Edges FL; .45, .45, .45, .45 (less); .55 (greater)")

plot(cs_array, title="County Splits FL; .45, .45, .45, .45 (less); .55 (greater)")

p = plot(title="Dem % Trace FL; .45, .45, .45, .45 (less); .55 (greater)")
for i in 20:28
