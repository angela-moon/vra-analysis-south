...
@everywhere function process_single_run(run_dir, i)
    ...
    return reduce(hcat, rows)', splits, cuts, reduce(hcat, brows)'
end
...
function process_one_dir(run_dir, num_runs, chain_idx, tmp_prefix="tmp_perc")
    ...
    jldsave(tmp_file; arr, splits_arr, cuts_arr, barr)
...
function compile_tmp_files(tmp_files, out_file, splits_out_file, cuts_out_file, bperc_out_file)
...
run_dirs = ["party_runs_SC/run_party4/"]
num_runs = 1000
tmp_files = String[]
...
compile_tmp_files(tmp_files, "SC_dperc5.jld2", "SC_splits5.jld2", "SC_cuts5.jld2", "SC_bperc5.jld2")
exit()

#### PROCESSING DATA
using StatsPlots, Plots

cs_array = load("SC_splits5.jld2", "county_splits_arr")
percents_array = load("./SC_dperc5.jld2", "perc_arr")
cut_edges_array = load("SC_cuts5.jld2", "cut_edges_arr")
bpercs_array = load("SC_bperc5.jld2", "barr")

key_percs = hcat(percents_array[:,160_000:175_000,1],percents_array[:,320_000:end,1])

b = plot(title="% Black Box Plots SC, beta_voteshare=750")

for i in 1:7
    boxplot!(b, ["D$i"], bpercs_array[i,:], label="D$i",legend=false,outliers=false)
end

scatter!(b, ["D1", "D2", "D3", "D4", "D5","D6","D7"], [0.1828165961087225,
 0.18492617005629175,
 0.19797997475993911,
 0.25449946333226225,
 0.2663038733124516,
 0.2726958300124814,
 0.4819280068798886], marker=:star5, markersize=8, color=:red)
#         label="Enacted")

hline!([0.5])
display(b)

b = plot(title="% Dem Box Plots SC, beta_voteshare=750")

for i in 1:7
    boxplot!(b, ["D$i"], percents_array[i,:], label="D$i",legend=false,outliers=false)
end

scatter!(b,  ["D1", "D2", "D3", "D4", "D5","D6","D7"], [0.28192461872153896,
 0.3668851460292951,
 0.3802530715562839,
 0.3849554396261276,
 0.42982538776550644,
 0.43424330297262426,
 0.6127263177906164], marker=:star5, markersize=8, color=:red)
#         label="Enacted")
...
plot(cut_edges_array, title="Cut Edges SC, beta_voteshare=750")

plot(cs_array, title="County Splits SC, beta_voteshare=750")

p = plot(title="Dem % Trace SC, beta_voteshare=750")
for i in 1:7
