...
function main(; initialization=nothing,N_ITERS=N_ITERS)
    ...
    energy_fn = make_combined_gaussian_party_energy(county_ids, 1.2, 0.01, 8, 650; df=df, k=K, targets=[0.30,0.30,0.30,0.30, 0, 0.55, 0.55, 0.55, 0.55, 0, 0, 0, 0, 0], uses=[:less, :less, :less, :equal, :do_nothing, :equal, :greater, :greater, :greater, :do_nothing, :do_nothing, :do_nothing, :do_nothing, :do_nothing], beta_voteshare=16, voteshare_slope_down=0.5)
...
serialize("cs_runs_GA/party2.jls", (a,b,c,d))