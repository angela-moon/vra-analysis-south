...
run_dirs = ["cs_runs_GA/run_party3"]
num_runs = 400
tmp_files = String[]
...
compile_tmp_files(tmp_files, "bpercents_GA_party4.jld2")

#### PROCESSING DATA
using StatsPlots, Plots

bperc_array = load("bpercents_GA_party4.jld2", "barr")
cs_array = load("county_splits_GA_party4.jld2", "county_splits_arr")
percents_array = load("./percents_GA_party4.jld2", "perc_arr")
cut_edges_array = load("edges_GA_party4.jld2", "cut_edges_arr")

c = open("cs_runs_GA/run_party3/run390.jls", "r") do io
            deserialize(io)
        end

ntd_vec = [c[1][node] for node in 1:nrow(df)]

scatter(percents_sc(df, c[1]), bpercents_sc(df, c[1]), title="Party/Race Scatter Plot, step 3900000", xlabel="Dem %", ylabel="Black Pop %")

key_percs = bperc_array[:,1_000_000:end,1]

b = plot(title="% Black Box Plots GA, beta_voteshare=750")

for i in 1:14
    boxplot!(b, ["D$i"], key_percs[i,:], label="D$i",legend=false,outliers=false)
end
display(b)

p = plot(title="% Black Trace GA, beta_voteshare=750")
for i in 1:14
    plot!(key_percs[i,:])
end
display(p)

b = plot(title="% Dem Box Plots GA, beta_voteshare=750")

for i in 1:14
    boxplot!(b, ["D$i"], percents_array[i,:], label="D$i",legend=false,outliers=false)
end
savefig(b, "./cs_runs_GA/dempercsbox.png")

edges = plot(cut_edges_array, title="Cut Edges GA, beta_voteshare=750")
savefig(edges, "./cs_runs_GA/cutedges.png")

cs = plot(cs_array, title="County Splits GA, beta_voteshare=750")
savefig(cs, "./cs_runs_GA/countysplits.png")

p = plot(title="% Dem Trace GA, beta_voteshare=750")
for i in 1:14
    plot!(percents_array[i,:,1])
end
savefig(p, "./cs_runs_GA/dempercstrace.png")

"""
p = plot(title="Dem % by district GA")
plot!(percents_array[i,:,1])
display(p)
