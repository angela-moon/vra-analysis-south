...
@everywhere function process_single_run(run_dir, i)
    ...
    return reduce(hcat, rows)', splits
end
...
function process_one_dir(run_dir, num_runs, chain_idx, tmp_prefix="tmp_perc")
    ...
    jldsave(tmp_file; arr, splits_arr)
...
function compile_tmp_files(tmp_files, out_file, splits_out_file)
...
compile_tmp_files(tmp_files, "percents_NC.jld2", "county_splits_NC.jld2")
exit()